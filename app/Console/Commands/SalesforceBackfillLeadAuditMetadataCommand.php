...
class SalesforceBackfillLeadAuditMetadataCommand extends Command
{
    ...
    public function handle(LeadRecordTypeNormalizer $types, LeadPortalResolver $portals): int
    {
        ...
        SalesforceLead::query()
            ...
            ->orderBy('id')
            ->chunkById($chunk, function ($rows) use ($types, $portals, $dryRun, &$processed, &$changed): void {
                foreach ($rows as $row) {
                    $processed++;
                    $payload = is_array($row->raw_payload) ? $row->raw_payload : [];
                    $resolution = $portals->resolve($row);
                    $updates = [
                        ...
                        'portal_resolution_source' => $resolution['source'],
